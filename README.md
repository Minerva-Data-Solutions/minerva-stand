<div align="center">
  <img src="nanobot_logo.png" alt="Minerva Stand" width="500">
  <h1>Minerva Stand</h1>
  <p>
    <img src="https://img.shields.io/badge/python-≥3.11-blue" alt="Python">
    <img src="https://img.shields.io/badge/license-MIT-green" alt="License">
    <img src="https://img.shields.io/badge/fork-nanobot-orange" alt="Forked from nanobot">
  </p>
</div>

**Minerva Stand** is Minerva's production-oriented fork of the lightweight `nanobot` agent runtime.

It keeps the small, hackable architecture that makes Nanobot appealing, then adds the control layers Minerva needs for real deployments: auditability, policy enforcement, and operational visibility.

For runtime compatibility today, the CLI command remains `nanobot`, the Python package remains `nanobot-ai`, and the default runtime directory remains `~/.nanobot`.

## Why Minerva Stand

Minerva Stand exists to turn the upstream Nanobot architecture into a Minerva-ready agent foundation without losing the simplicity of the original codebase.

What Minerva adds on top of the base project:

- Automatic SQLite audit logging for agent runs and tool activity
- A config-driven guardrails layer that validates tool calls before execution
- Retention-based cleanup for audit data
- CLI inspection commands: `nanobot agent runs` and `nanobot agent logs`
- Minerva-specific documentation, examples, and operational guidance
- A more deployment-oriented framing for configuration, security, and observability

Minerva Stand is still built on the Nanobot architecture, but the project is maintained and documented here as a Minerva distribution first.
...
- [Why Minerva Stand](#why-minerva-stand)
...
> [!IMPORTANT]
> Minerva Stand currently keeps upstream runtime compatibility.
> That means the installed package is still `nanobot-ai`, the CLI command is still `nanobot`, and local state still lives under `~/.nanobot`.

**Install from source** (recommended for Minerva development)
...
**Install with [uv](https://github.com/astral-sh/uv)** (runtime-compatible install)

```bash
uv tool install nanobot-ai
```

**Install from PyPI** (runtime-compatible install)
...
> [!TIP]
> Minerva Stand currently uses the compatible runtime config path `~/.nanobot/config.json`.
...
That's it! You have a working Minerva agent runtime in a couple of minutes.

## 💬 Chat Apps

Connect Minerva Stand to your preferred chat platform. Want to build your own? See the [Channel Plugin Guide](./docs/CHANNEL_PLUGIN_GUIDE.md).
...
Runtime config file: `~/.nanobot/config.json`

Minerva Stand currently keeps the upstream-compatible runtime path so existing scripts and environments continue to work unchanged.
...
> Minerva Stand will merge in missing default fields and keep your current settings.
...
Minerva Stand can automatically audit every run to SQLite and validate every tool call against a JSON guardrails policy before execution.
...
Policy files are JSON-only. If the configured file is missing, Minerva Stand falls back to built-in defaults for the core tools.

Example policy: [`docs/guardrails.example.json`](docs/guardrails.example.json)
...
nanobot agent runs
nanobot agent logs
nanobot agent logs --run-id <run-id>
```
...
> - **MiniMax Coding Plan**: Useful discount links for Minerva users: [Overseas](https://platform.minimax.io/subscribe/coding-plan?code=9txpdXw04g&source=link) · [Mainland China](https://platform.minimaxi.com/subscribe/token-plan?code=GILTJpMTqZ&source=link)
...
| `nanobot agent runs` | Show recent audited runs |
| `nanobot agent logs` | Show recent audited events |
...
Use Minerva Stand as a library — no CLI, no gateway, just Python:
...
Minerva Stand can expose a minimal OpenAI-compatible endpoint for local integrations:
...
> [!TIP]
> The `-v ~/.nanobot:/home/nanobot/.nanobot` flag mounts the compatible Minerva runtime directory into the container, so your config and workspace persist across container restarts.
...
Run the Minerva gateway as a systemd user service so it starts automatically and restarts on failure.
...
├── guardrails/     # 🛡️ Tool policy and validation layer
...
Minerva Stand is maintained as a Minerva-owned fork. Contributions are welcome, especially where they improve reliability, observability, and policy control without bloating the core runtime.

Before contributing, read [CONTRIBUTING.md](./CONTRIBUTING.md) for workflow details.
...
- [ ] **More integrations** — Internal tools, workspace systems, and channel extensions
- [ ] **Operational controls** — Stronger audit, policy, and runtime governance
- [ ] **Self-improvement** — Learn from feedback and mistakes

<p align="center">
  <em>Built and maintained by Minerva.</em>
</p>


<p align="center">
  <sub>Minerva Stand is built on a maintained fork of nanobot and keeps upstream runtime compatibility where practical.</sub>
</p>
