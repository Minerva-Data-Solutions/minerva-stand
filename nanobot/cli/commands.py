...
def serve(
    ...
    agent_loop = AgentLoop(
        ...
        audit_config=runtime_config.audit,
        guardrails_config=runtime_config.guardrails,
...
agent_app = typer.Typer(
    help="Interact with the agent and inspect audit logs",
...
def agent_runs(
    ...
    """Show recent audited agent runs."""
    from nanobot.audit.formatters import build_empty_text, build_runs_table, query_runs

    runtime_config = _load_runtime_config(config)
    rows = query_runs(
        runtime_config.audit_db_path,
        limit=limit,
        status=status,
        agent_id=agent_id,
    )
    if not rows:
        console.print(build_empty_text("agent runs"))
        return
    console.print(build_runs_table(rows))


@agent_app.command("logs")
def agent_logs(
    limit: int = typer.Option(50, "--limit", "-n", min=1, help="Maximum logs to show"),
    ...
    tool_name: str | None = typer.Option(None, "--tool", help="Filter by tool name"),
    status: str | None = typer.Option(None, "--status", help="Filter by event status"),
    config: str | None = typer.Option(None, "--config", "-c", help="Config file path"),
):
    """Show recent audited agent events."""
    from nanobot.audit.formatters import build_empty_text, build_events_table, query_events
    ...
    if not rows:
        console.print(build_empty_text("agent logs"))
        return
    console.print(build_events_table(rows))
