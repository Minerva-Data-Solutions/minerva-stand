...
class AuditConfig(Base):
    ...
    enabled: bool = True
    db_path: str = str(Path.home() / ".nanobot" / "audit" / "agent_audit.db")
...
class GuardrailsConfig(Base):
    ...
    enabled: bool = True
    policy_path: str = str(Path.home() / ".nanobot" / "guardrails.json")
...
class Config(BaseSettings):
    ...
    def audit_db_path(self) -> Path:
        """Get expanded audit database path."""
        return Path(self.audit.db_path).expanduser()

    @property
    def guardrails_policy_path(self) -> Path:
        """Get expanded guardrails policy path."""
        return Path(self.guardrails.policy_path).expanduser()
