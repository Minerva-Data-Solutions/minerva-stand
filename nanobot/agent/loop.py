...
class AgentLoop:
    ...
    def __init__(
        ...
        self.audit_config = audit_config or AuditConfig(enabled=False)
        self.guardrails_config = guardrails_config or GuardrailsConfig(enabled=False)
        ...
        self.agent_id = str(
            uuid5(
                NAMESPACE_URL,
                f"{self.workspace.expanduser().resolve(strict=False)}::{self.model}",
            )
        )
        ...
        self.audit_writer = AuditWriter(self.audit_config) if self.audit_config.enabled else None
        self.guardrails_loader = (
            GuardrailsLoader(self.guardrails_config)
    ...
    async def _run_agent_loop(
        ...
        *on_stream_end(resuming)*: called when a streaming session finishes.
        ...
        if self.audit_writer is not None:
            run_id = await self.audit_writer.start_run(self.agent_id)
            if run_id is not None:
                from nanobot.audit.hook import AuditHook
        ...
        hook: AgentHook = (
            CompositeHook(
                [item for item in [loop_hook, audit_hook, *self._extra_hooks] if item is not None]
            )
            if audit_hook is not None or self._extra_hooks
            else loop_hook
        )
        ...
        if self.audit_writer is not None or self.guardrails_loader is not None:
            ...
            tool_interceptor = ToolExecutionInterceptor(
                ...
                guardrails_loader=self.guardrails_loader,
            )
        try:
            ...
            if self.audit_writer is not None and run_id is not None:
                await self.audit_writer.finish_run(
                    run_id,
                    status="error",
                    total_steps=audit_hook.total_steps if audit_hook is not None else 0,
                )
            raise
        if self.audit_writer is not None and run_id is not None:
            await self.audit_writer.finish_run(
