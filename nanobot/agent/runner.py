...
class AgentRunSpec:
    ...
    tool_interceptor: Any | None = None
...
class AgentRunner:
    ...
    async def _run_tool(
        ...
        if lookup_error:
            interceptor = spec.tool_interceptor
            if interceptor is not None:
                await interceptor.record_blocked(
                    tool_name=tool_call.name,
                    params=tool_call.arguments,
                    message=lookup_error,
                    reason_code="repeated_external_lookup",
                )
            event = {
                "name": tool_call.name,
                "status": "error",
            ...
            if spec.fail_on_tool_error:
                return lookup_error + error_hint, event, RuntimeError(lookup_error)
            return lookup_error + error_hint, event, None
        ...
        if prep_error:
            ...
            if interceptor is not None:
                await interceptor.record_blocked(
                    ...
                    message=prep_error,
                    reason_code="prepare_call_error",
                )
            event = {
                "name": tool_call.name,
                "status": "error",
            ...
            return prep_error + error_hint, event, RuntimeError(prep_error) if spec.fail_on_tool_error else None
        interceptor = spec.tool_interceptor
        if interceptor is not None:
            async def _invoke() -> Any:
                if tool is not None:
                    return await tool.execute(**params)
                return await spec.tools.execute(tool_call.name, params)

            outcome = await interceptor.execute(
                tool_name=tool_call.name,
                params=params,
                invoke=_invoke,
            ...
            return outcome.result, outcome.event, outcome.error
        try:
            if tool is not None:
        ...
        if isinstance(result, str) and result.startswith("Error"):
            ...
            if spec.fail_on_tool_error:
                return result + error_hint, event, RuntimeError(result)
            return result + error_hint, event, None
