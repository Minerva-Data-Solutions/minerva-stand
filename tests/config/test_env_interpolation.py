...
class TestResolveConfig:
    ...
    def test_load_config_resolves_relative_audit_and_guardrails_paths(self, tmp_path):
        config_path = tmp_path / "instance" / "config.json"
        config_path.parent.mkdir(parents=True)
        config_path.write_text(
            json.dumps(
                {
                    "audit": {"dbPath": "runtime/audit.db"},
                    "guardrails": {"policyPath": "policies/guardrails.json"},
                }
            ),
            encoding="utf-8",
        )
        ...
        assert config.audit_db_path == config_path.parent / "runtime" / "audit.db"
        assert config.guardrails_policy_path == config_path.parent / "policies" / "guardrails.json"
