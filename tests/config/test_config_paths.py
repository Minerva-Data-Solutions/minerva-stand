...
def test_shared_and_legacy_paths_remain_global() -> None:
    assert get_audit_dir() == Path.home() / ".nanobot" / "audit"
    assert get_audit_db_path() == Path.home() / ".nanobot" / "audit" / "agent_audit.db"
    assert get_guardrails_policy_path() == Path.home() / ".nanobot" / "guardrails.json"
