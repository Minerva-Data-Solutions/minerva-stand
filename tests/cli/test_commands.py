...
def test_agent_runs_command_prints_audit_runs(monkeypatch, tmp_path: Path) -> None:
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        conn.execute(
            ...
            ("run-1", "agent-1", "2026-04-13T00:00:00+00:00", "2026-04-13T00:01:00+00:00", "completed", 3),
        )
        conn.commit()

    config = Config()
    config.audit.db_path = str(db_path)
    ...
    assert result.exit_code == 0
    stripped_output = _strip_ansi(result.stdout)
    assert "run-1" in stripped_output
    assert "completed" in stripped_output


def test_agent_logs_command_prints_audit_events(monkeypatch, tmp_path: Path) -> None:
    db_path = tmp_path / "agent_audit.db"
    with sqlite3.connect(db_path) as conn:
        conn.execute(
            """
    ...
    stripped_output = _strip_ansi(result.stdout)
    assert "run-1" in stripped_output
    assert "exec" in stripped_output
...
def _patch_serve_runtime(monkeypatch, config: Config, seen: dict[str, object]) -> None:
    ...
    def _fake_create_app(agent_loop, model_name: str, request_timeout: float, *, workbench=None):
        seen["agent_loop"] = agent_loop
        seen["model_name"] = model_name
        seen["request_timeout"] = request_timeout
        seen["workbench"] = workbench
        return _FakeApiApp()
